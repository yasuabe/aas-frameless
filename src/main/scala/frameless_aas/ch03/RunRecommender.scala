...
//    implicit val spark = SparkSession.builder().config(conf).appName("aas-frameless").getOrCreate()
...
case class T3(a: Int, b: Int, c: Int)

    case class UserArtist(a: Int, b: Int, c: Int)
    val a = implicitly[As[(Int, Int, Int), T3]]

    val rawUserArtistData = spark.read.textFile("user_artist_data.txt")
    val rawArtistData = spark.read.textFile("artist_data.txt")
...
class RunRecommender(private val spark: SparkSession) {
  ...
  def areaUnderCurve(
      ...
      predictFunction: (DataFrame => DataFrame)): Double = { // predictFunction: (artist, user)* => (artist, user, prediction)*
